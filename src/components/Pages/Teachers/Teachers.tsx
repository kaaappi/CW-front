...
const Teachers: React.FC = () => {
  ...
  const handleTeachers = async () => {
    const response = await axios.get(
      `http://localhost:3000/getTeachers?Param=${selectedOptionForQuery}`
    );
    ...
    if (conf) {
      try {
        ...
        const response = await axios.delete(
          "http://localhost:3000/deleteTeacher",
          {
            params: {
  ...
  // ==================================для условной отрисовки + put запрос===========
  ...
  const handleUpd = async (
    ...
    try {
      const response = await axios.put(
        "http://localhost:3000/editTeacherByFullName",
        {
          LastName: lastName,
          FirstName: firstName,
