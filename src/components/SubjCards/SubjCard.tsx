...
const SubjCard: FC<SubjCardProps> = ({
  ...
  const handleGroups = async () => {
    try {
      const response = await axios.get(`http://localhost:3000/getGroupsByFullNameAndSubj?fullName=${fullName}&subjectName=${subject}`);
  ...
  const handleSaveClick = () => {
    ...
    axios
      .put("http://localhost:3000/editLoadByFullName", dataToSend)
      .then((response) => {
    ...
    if (conf) {
      try {
        const response = await axios.delete(
          `http://localhost:3000/deleteLoadByFullNameAndGroup`,
          {
            params: {
              TeacherFullName: fullName,
              GroupCode: editedGroups,
